...
// Mock VIP Booking Assistant for demo purposes
class MockVIPBookingAssistant {
  ...
  async processMessage(message) {
    ...
    let response = "I understand you're interested in our VIP transport services. ";
    
    if (lowerMessage.includes('airport')) {
      response += "Our airport transfer service ensures punctual, comfortable journeys. What's your preferred pickup location and travel date?";
    } else if (lowerMessage.includes('wedding')) {
      response += "Congratulations! We'll make your special day even more memorable with our luxury wedding transport. When is your wedding date?";
    } else if (lowerMessage.includes('corporate')) {
      response += "Our corporate transport service provides professional, reliable travel solutions. What type of corporate event or meeting do you need transport for?";
    } else {
      response += "I'd be happy to help arrange your luxury transport. Could you tell me more about your travel needs?";
    }

    if (lowerMessage.includes('john') || lowerMessage.includes('name')) {
      this.extractedData.customer_name = 'John';
    }
    if (lowerMessage.includes('@') || lowerMessage.includes('email')) {
      this.extractedData.customer_email = 'customer@example.com';
    }

    return {
      response,
      bookingReady: Object.keys(this.extractedData).length > 3
    };
  }
  ...
  async submitBooking() {
    return { success: true, bookingId: 'VIP-' + Date.now() };
  }
}
...
const AIBookingAssistant = ({ 
  ...
  useEffect(() => {
    if (!assistantRef.current) {
      assistantRef.current = new MockVIPBookingAssistant(sessionId);
    }
  }, [sessionId]);
  ...
  const getTypingDelay = (messageLength) => {
    const baseDelay = 800;
      ...
      <div className="space-y-1.5">
        {parts.map((part, index) => {
          ...
          if (part.startsWith('📞') || part.startsWith('📧')) {
            return (
              <div key={index} className="bg-blue-50 p-2 rounded text-xs sm:text-sm text-blue-800 mt-1">
                {part}
              </div>
            );
            ...
            return (
              <div key={index} className="flex items-start space-x-2 ml-2">
                <span className="text-blue-500 mt-0.5 flex-shrink-0 text-xs sm:text-sm">
                  {part.startsWith('•') ? '•' : part.charAt(0)}
                </span>
                <span className="text-xs sm:text-sm">{part.slice(part.startsWith('•') ? 1 : 2).trim()}</span>
              </div>
            );
          } else {
            return <div key={index} className="text-xs sm:text-sm">{part}</div>;
          }
        })}
      </div>
    );
  ...
  if (isMobile && isOpen) {
    return (
      <div className="fixed inset-0 z-50 bg-white flex flex-col">
          ...
          <div className="bg-green-50 border-b border-green-200 p-3">
            <div className="flex items-center justify-between text-green-700 text-sm">
              <div className="flex items-center space-x-2">
                ...
                <span>Progress: {completedFields.length}/11 details</span>
              </div>
              <div className="flex items-center space-x-2">
                <span className="text-xs">{getProgressPercentage()}%</span>
        ...
        {/* Error display */}
        {error && (
          <div className="bg-red-100 text-red-800 p-3 text-sm mx-4 mt-2 rounded-md flex items-center space-x-2">
            <X className="w-4 h-4" />
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
          {messages.map((message, index) => (
            ...
            >
              <div
                className={`max-w-[90%] px-3 py-2 rounded-2xl shadow-sm ${
                  message.role === 'user'
        ...
        <div className="border-t border-gray-200 p-4 bg-white">
          <div className="flex space-x-3">
            <input
              ...
              className="flex-1 px-4 py-3 border border-gray-300 rounded-full focus:ring-2 focus:ring-blue-400 focus:border-blue-400 text-sm transition-all duration-200"
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {['Airport Transfer', 'Wedding Transport', 'Corporate Travel'].map((option) => (
                <button
                  ...
                  className="px-3 py-1.5 text-sm bg-gray-100 hover:bg-blue-100 text-gray-700 hover:text-blue-700 rounded-full transition-colors duration-200"
                  disabled={isLoading || isTyping}
                >
                  {option}
                </button>
          ...
          {/* Footer - Mobile */}
          <div className="flex flex-col items-center mt-3 text-xs text-gray-500 space-y-1">
            <div className="flex items-center space-x-2">
  ...
  return (
    <>
      {/* Floating Button */}
      {!isOpen && (
        <div className={`fixed bottom-4 sm:bottom-6 ${positionClasses[position]} z-50`}>
          <button
            onClick={() => setIsOpen(true)}
            className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white p-3 sm:p-4 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 group relative"
            aria-label="Open VIP booking assistant"
          >
            <MessageCircle className="w-6 h-6" />
            <div className="absolute -top-1 -right-1 bg-yellow-400 text-black text-xs rounded-full w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center animate-pulse font-bold">
              <Sparkles className="w-3 h-3" />
            </div>
            <div className="absolute -bottom-12 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs px-3 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap hidden sm:block">
      ...
      {isOpen && !isMobile && (
        <div
          className={`fixed bottom-4 sm:bottom-6 ${positionClasses[position]} z-50 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden transition-all duration-300`}
          style={{
            width: '400px',
              ...
              <div className="border-t border-gray-200 p-4 bg-white">
                <div className="flex space-x-3">
                  ...
                  <button
                    onClick={handleSendMessage}
                    disabled={isLoading || isTyping || !inputMessage.trim()}
                    className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white p-3 rounded-full transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:scale-105"
                  >
                    <Send className="w-5 h-5" />
                  </button>
                </div>
      ...
      {/* Backdrop for desktop when needed */}
      {isOpen && !isMobile && (
        <div
          className="fixed inset-0 bg-black bg-opacity-10 z-40 hidden sm:block"
          onClick={() => setIsOpen(false)}
        />
      )}
    </>
