...
const AIBookingAssistant: React.FC<AIBookingAssistantProps> = ({ 
  ...
  const getTypingDelay = (messageLength: number): number => {
    // More natural typing delays based on message length
    const baseDelay = 800;
            ...
            return part.split('\n').map((line, lineIndex) => (
              <div key={`${index}-${lineIndex}`}>
                {line.startsWith('•') ? (
                  ...
                  <div className="bg-blue-50 p-2 rounded text-sm text-blue-800 mt-1">
                    {line}
                  </div>
                ) : (
                  line && <div>{line}</div>
...
export default AIBookingAssistant;</div>